...
def train(inp,target,flag):
	...
	global net
	print type(inp)
	net = nl.net.newff([[0, 1]] *129, [16, 26])		#128 input nodes,16 hidden nodes,26 output nodes
	...
	err = net.train(inp, target, show=1,epochs=75)	#Gradient Descent Backpropagation	
	'''for i in range(len(inp)):				#checking training error
		train_arr=net.sim(inp[i])		
		print target[i],train_arr'''
	net.save('netTrainedhyb16.txt')

def test(test_input):
	#net=nl.load('netTrained16_.net')
...
if __name__ == '__main__':
	...
	'''for i in inp:
		...
		#print temp
		out[temp[1]]={}'''
	l=eval(open('hmmouttrain.txt').read())
	k=eval(open('hmmout.txt').read())
	for i in inp:
		temp=i.split()
		temp_1=list(map(float,temp[6:]))
		temp_1.append(float(l[j]))
		out.append(temp_1)
	...
	for i in test_temp:
		temp=i.split()
		temp_2=list(map(float,temp[6:]))
		temp_2.append(float(k[j]))
		test_data[temp[1]][int(temp[0])]=temp_2
		j+=1
